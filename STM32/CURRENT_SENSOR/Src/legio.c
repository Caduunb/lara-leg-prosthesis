...
void write_pwm_freq_to_sd(float frequency, char string[]){ //writes string
	char buffer[50];

	sprintf(buffer, "PWM frequency set to %.2f khz\r\n", frequency); //see if writing works
	HAL_UART_Transmit(&huart1, (uint8_t *)buffer, strlen(buffer), 30);
}

void write_sampled_data_current_sensor(int v_avg, int dutycycle, int adc_value){
	char buffer[100];
